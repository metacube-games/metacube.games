...
/**
 * Mirrors the canonical setup from market / link-wallet so the
 * `starknetkit` picker in the login dialog lists the same connectors
 * (Argent / Braavos / Ready / etc. + Cartridge `ControllerConnector`).
 */
    ...
    useEffect(() => {
      ...
      const initializeConnector = async () => {
        try {
          ...
          // Cartridge connector failed to initialize — fall back to the
          // other connectors (Argent / Braavos / injected).
        }
      };
      initializeConnector();
    }, []);
