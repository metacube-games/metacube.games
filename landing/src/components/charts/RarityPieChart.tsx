...
export default function RarityPieChart({ title, data }: RarityPieChartProps) {
  ...
  const createPieChartSegments = () => {
    ...
    const totalValue = Object.values(data)
      .flat()
        ...
        segments.push(
          <div
            ...
            style={{
              ...
              clip: percentage > 50 ? "unset" : "rect(0, 150px, 150px, 75px)",
  ...
  return (
    <div className="w-full mb-12 p-6 rounded-xl bg-gradient-to-br from-zinc-900/80 to-black border-2 border-zinc-800">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-center">
        ...
        <div className="grid grid-cols-2 gap-3 w-full">
          {Object.entries(data).map(([rarity, items]) =>
            items.map((item, idx) => (
              <div
                key={`legend-${rarity}-${idx}`}
                className="flex items-center gap-2"
              >
                <div
                  className={`w-4 h-4 rounded-sm ${getRarityClass(rarity)}`}
                ></div>
