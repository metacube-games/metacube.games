...
export default function NFTStatsSection() {
  ...
  return (
    <div className="space-y-12">
      {/* Introduction */}
      <div className="p-6 rounded-xl bg-gradient-to-br from-gray-900 to-black border-2 border-gray-800">
        <h3 className="text-2xl font-bold mb-4">{t("title")}</h3>
        <p className="text-gray-300 mb-4">{t("description")}</p>
        <div className="flex flex-wrap gap-3">
          <div className="px-3 py-1 bg-slate-800 rounded-full text-xs">
            {t("totalNfts")}
          </div>
          <div className="px-3 py-1 bg-slate-800 rounded-full text-xs">
            {t("traitCategories")}
      ...
      <div>
        ...
        <RarityBarChart
          title={t("skinDistribution")}
          data={nftStatistics.Skin}
        />
      </div>
      ...
      <div>
        ...
        <RarityBarChart
          title={t("cubeDistribution")}
          data={nftStatistics.Cube}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div>
          ...
          <RarityBarChart
            title={t("coinDistribution")}
            data={nftStatistics.Coin}
          />
        </div>
        ...
        <div>
          ...
          <RarityBarChart
            title={t("backgroundDistribution")}
            data={nftStatistics.Background}
          />
        </div>
        ...
        <div>
          ...
          <RarityBarChart
            title={t("weaponDistribution")}
            data={nftStatistics.Weapon}
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {["common", "rare", "epic", "mythic", "legendary"].map((rarity) => (
            <div key={rarity} className="text-center">
              <div
                className={`w-12 h-12 mx-auto rounded-full mb-2 ${getRarityClassShort(rarity)}`}
              ></div>
